...
- [~] `omega3p_ace3p_param_sweep` — run 1 (job 37571138, 7m54s, after ~1.5 h in queue): 32 wide rows, no NaNs, Sigma axis as its own column, R/Q and E_max identical across Sigma (as physics says). BUT 3 of 32 points had `Mode_freq` ≈ 2.2 GHz (the 2nd mode): Omega3P wrote its `Mode` blocks first in `omega3p.out` and the un-stripped `/* */` header swallowed the first one → fixed in b855423 (fixture added). Run 2 (job 37602502) submitted to confirm. **Update when done.**
