...
- [x] `s3p_sweep` — job 37559395, 6m18s, 15 points, 195 rows, no NaNs. **Re-validated 2026-09-10 after 6faaf98** (scan 9.5–12.5 GHz, four S(m,n) outputs + `reflection_12GHz`): job 37747638, 6m18s, 195 rows × 8 cols, no NaNs, 13 scan points incl. 12.0 GHz, scalar equals S(0,0)@12 GHz at every point, |S00|²+|S01|² = 1.0000, reflection_12GHz 0.007–0.19 (min at cornercut 14, rcorner2 16).

### Sweeps (short, do these first)

- [x] `s3p_sweep_no_s3p_file` — job 37561378, 6m13s, 15 points, 195 rows, no NaNs; SParameter.out sane (|S|≤1, |S11|²+|S21|²≈1). (At that time the YAML declared no `output_parameters`, so the table was only `cornercut, rcorner2, Frequency`.) **Re-validated 2026-09-10 after 6faaf98**: job 37748242, 6m21s, 195 rows × 8 cols, no NaNs, table identical to the re-validated `s3p_sweep` table to the last digit.
...
- [x] `s3p_optimization` — run 2 (job 37727718, 7m04s): 25 NM evaluations, no NaNs/errors, |S11| 0.117 → 0.020, converging to the bound corner (cornercut 14, rcorner1 2.5). Run 1 (37726476, 8m04s) completed with exit 0 but **every objective was NaN**: the YAML asked for `at: {frequency: 12.0e9}` and the .s3p scan is 9.424 + k·0.25 GHz (no 12.0). Example moved to 11.924 GHz and an off-grid frequency now raises (64c45b3). **Re-validated 2026-09-10 after 6faaf98** (scan moved to 9.5–12.5 GHz, objective back at 12.0 GHz): job 37748676, 7m14s, 25 evaluations, no NaNs/errors, reflection 0.120 → 0.0195, same bound corner (cornercut 14, rcorner1 2.5). Reflection.out confirms 1.2e10 Hz is a scan point.
...
- [x] `s3p_bayesian_sweep` — job 37731398, 3m01s (8 ranks): 13 real S3P evaluations in `sim_output.txt` (|S11| 0.004–0.095), 100-point GP grid in `sweep_output.txt` (means 0.00–0.095), no NaNs/errors. A torch `requires_grad` → `float()` warning per grid point silenced with `.detach()`. **Re-validated 2026-09-10** after the `max_steps` fix (the YAML's `num_step: 3` was never read by `gp_parameter_sweep`, and the loop ran one step past `max_steps`): job 37751697, 1m49s, exactly 8 evaluations (5 random + 3 GP steps), 100-point GP grid, no NaNs/errors.
