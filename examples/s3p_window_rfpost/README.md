...
window and reports its S-parameters, with `acdtool postprocess rf` run on the
**S3P** solution.
...
**`postprocess rf` on S3P output.** Every other acdtool example postprocesses an
Omega3P eigensolve. `window.rfpost` points at the S3P results:
...
the third, 2.556 GHz. For S3P, `ModeID` means a **port mode** (an excitation),
not an eigenmode. `gradient = -1` means "no field scaling", which selects the
point-scaled variant of the `[scaling]` block in the output.

**Two index axes, one table.** S3P's results are indexed by `Frequency` and
acdtool's by `ModeID`; a flat table carries one index, and *the first producer in
resolved DAG order wins*, so the table is frequency-indexed. acdtool's per-mode
arrays and `ALLFieldOnLine` curves are not table columns: they are read into the
acdtool module's output, available through its `field()`, and the curve files
stay in each workdir (`field1_0`, `field1_0.ec`, `field1_0.bc`, …). A
frequency-indexed sweep writes no per-row field artifact `.npz`, since the rows
already are the S3P spectrum. To use the acdtool curves in a post-processing
script, read them from the workdir.
...
because a `cubit` module plays exactly one journal. Dropping the ACIS round-trip
also drops the surface numbering the meshing half relied on, so the joined
journal selects ports and symmetry planes **by position**
(`surface with z_coord < ...`, `x_coord`, `y_coord`) rather than by ID, and
excludes merged interior surfaces from every boundary sideset. A vacuum/ceramic
interface in a symmetry sideset makes `acdtool meshconvert` flag a bad Euler
characteristic and S3P abort.
...
**Budget for it.** Three geometries × a 16-point frequency scan is 48 separate
S3P solves, the heaviest of the three RF examples here; it does not fit the
30-minute debug QOS.
...
port 8 mode 0, so `S(0,0)` is the reflection and `S(0,1)` the transmission. For
your own model, check the `#Index mapping:` header of
`s3p_results/Reflection.out`.
...
* **An optimization.** `S11_at_scan_point` is the scalar-at-a-frequency form an
  Xopt objective needs: minimize it from a `scalar_optimize` mode's VOCS. Each
  evaluation is a full frequency scan, so consider narrowing `FrequencyScan`
  first.
* **Sweeping S3P settings** rather than geometry: put them under
  `input_parameters: ace3p:` addressed by their path in the input file.
