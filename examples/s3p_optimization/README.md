...
evaluated `at` frequency `11.924e+09`, a point of the `.s3p` frequency scan
(9.424–12.424 GHz in 0.25 GHz steps) — so the Xopt driver never parses
...
- `bend-90degree.s3p` — the S3P/ACE3P config (order-2 curved elements, two
  waveguide ports, and the `FrequencyScan` 9.424–12.424 GHz in 0.25 GHz
  steps; the objective's `at:` frequency must be one of its points).
